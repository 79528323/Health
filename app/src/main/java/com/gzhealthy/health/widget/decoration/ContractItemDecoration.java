 ...
 */
public class ContractItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public ContractItemDecoration(Context context) {
    ...
    public ContractItemDecoration(Context context, int top) {
        final TypedArray a = context.obtainStyledAttributes(ATTRS);
        mDivider = a.getDrawable(0);
        this.top = top;
        a.recycle();
...
//        }
    ...
    public void getItemOffsets(Rect outRect, int itemPosition, RecyclerView parent) {
        int bottom = 0;
        int size = ((ContractAdapter)parent.getAdapter()).mList.size() - 1;
        if (size == itemPosition){
            bottom = padding;
        }
        outRect.set(0, padding, 0, bottom);
    }
}