...
public class XAxisRenderer extends AxisRenderer {
    ...
    protected void drawLabels(Canvas c, float pos, MPPointF anchor) {
        ...
        for (int i = 0; i < positions.length; i += 2) {
            ...
            if (mViewPortHandler.isInBoundsX(x)) {
                ...
                if (mXAxis.isDrawXaxisCustomValues()){
                    drawLabel(c, label, x, pos + Utils.convertDpToPixel(14f), anchor, labelRotationAngleDegrees);

                    try {
                        //TODO 自定义步数底部Label
                        if (!mXAxis.getmLabelList().isEmpty()){
                            String date = mXAxis.getmLabelList().get(i/2);
                            date = Utils.convertDate(date);
                            drawLabel(c, date , x , pos ,anchor, labelRotationAngleDegrees);
                        }
                    }catch (Exception e){
                        e.printStackTrace();
                    }
    ...
    public void renderGridLines(Canvas c) {
        ...
        for (int i = 0; i < positions.length; i += 2) {
            if (mXAxis.isDrawCustomRateLine()){
    ...
    public void renderScaleLines(Canvas c){
        if (!mXAxis.ismIsDrawScale() || !mXAxis.isEnabled()){
            return;
        }
    ...
    protected void drawScale(Canvas canvas,float startX , float offset){
        ...
        if (mXAxis.getPosition() == XAxisPosition.BOTTOM) { //X轴位置在下方时
            for (int i = 0; i <= 5; i++) {
                ...
                if (i % 5 == 0) {
                    mAxisLinePaint.setStrokeWidth(Utils.convertDpToPixel(1f));
                    canvas.drawLine(startX, bottomY + 10, startX, bottomY, mAxisLinePaint);//画长刻度线
                }
//                else {
