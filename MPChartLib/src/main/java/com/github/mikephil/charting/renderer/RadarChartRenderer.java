...
public class RadarChartRenderer extends LineRadarRenderer {
     ...
     * 圆点画笔
     */
    protected Paint mWebPointPaint;

    /**
     * 自定义样式 透明度
     */
    private final String[] radarAlpha={"#FFC7EAE6","#8CC7EAE6","#4DC7EAE6","#80DAF6F3","#4DDAF6F3","#26DAF6F3"};
    ...
    public RadarChartRenderer(RadarChart chart, ChartAnimator animator,
        ...
        mWebPointPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mWebPointPaint.setStyle(Paint.Style.FILL);
        mWebPointPaint.setColor(Color.parseColor("#00D0B8"));
    }
    ...
    protected void drawDataSet(Canvas c, IRadarDataSet dataSet, int mostEntries) {
        ...
        for (int j = 0; j < dataSet.getEntryCount(); j++) {
            ...
            if (mChart.isRadarCustomStyle()){
                Paint circlePaint = new Paint();
                circlePaint.setColor(Color.parseColor("#00D0B8"));
                c.drawCircle(pOut.x, pOut.y,8,circlePaint);
            }
        }
    ...
    protected void drawWeb(Canvas c) {
        ...
        for (int i = 0; i < maxEntryCount; i += xIncrements) {
            ...
            //TODO 禁用中心点直线绘制
            if (!mChart.isNoRenderXAisLine()) {
                c.drawLine(center.x, center.y, p.x, p.y, mWebPaint);
            }

            if (mChart.isRenderXAisCircle()){
                c.drawCircle(p.x , p.y , 8 , mWebPointPaint);
            }
        }
        ...
        for (int j = 0; j < labelCount; j++) {
            ...
            for (int i = 0; i < mChart.getData().getEntryCount(); i++) {
                ...
                if (mChart.isRadarCustomStyle()){
                    if (i == 0){
                        fillPath.moveTo(p1out.x,p1out.y);
                        fillPath.lineTo(p2out.x,p2out.y);
                    }else {
                        fillPath.lineTo(p2out.x,p2out.y);
                    }
                }
            ...
            if (mChart.isRadarCustomStyle()){
                ...
                mWebPaint.setColor(Color.parseColor(radarAlpha[j]));
                c.drawPath(fillPath,mWebPaint);
            }
        }
