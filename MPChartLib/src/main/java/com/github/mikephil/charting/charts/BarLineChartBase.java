...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
     ...
     * 是否自定义边界样式
     */
    private boolean isDrawCustomBorders = false;

    public boolean isDrawCustomBorders() {
        return isDrawCustomBorders;
    }

    public void setDrawCustomBorders(boolean drawCustomBorders) {
        isDrawCustomBorders = drawCustomBorders;
    }
    ...
    protected void drawGridBackground(Canvas c) {
        ...
        if (mDrawBorders) {
            //自定义边界线样式
            if (isDrawCustomBorders){
                mBorderPaint.setStrokeWidth(3f);
                mBorderPaint.setAntiAlias(true);
                mBorderPaint.setColor(Color.LTGRAY);
                c.drawRoundRect(mViewPortHandler.getContentRect(),isRadius?20f:0f,isRadius?20f:0f,mBorderPaint);
            }else
